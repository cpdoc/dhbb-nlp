...
awk '$4 ~ /VERB/ {print $3}' *.conllu | sort | uniq -c | awk 'BEGIN {OFS="\t"} {print $2,$1}' | sort > verbs.dhbb
awk '$4 ~ /VERB/ {print tolower($2),$3}' *.conllu | sort | uniq -c | awk 'BEGIN {OFS="\t"} {print $2,$3,$1}' | sort > tverbs.dhbb
...
awk '{split($2,a,/\+/); print $1,a[1]}' $MORPHO_DIR/*.dict | sort | uniq -c | awk 'BEGIN {OFS="\t"} {print $2,$3,$1}' | sort  > tverbs.morpho

# cria um arquivo com os lemas que estão em verbs.verb e não estão no
# morpho.verb
join -t \t -v 1 verbs.dhbb verbs.morpho > verbs-missing-by-lemma.tsv

# now by form instead of lemma
join -t \t -v 1 tverbs.dhbb tverbs.morpho > verbs-missing-by-form.tsv
